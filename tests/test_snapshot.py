...
class SnapshotTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.policy = MediaPolicy()
        self.service = SnapshotHttpService(
            "127.0.0.1",
            0,
            "spruthub",
            "rtsp-password",
            GetCameraSnapshot(MemoryRepository(state), self.gateway, self.policy),
        )
        self.service.start()
    ...
    def test_authenticated_snapshot_is_returned_on_demand(self) -> None:
        ...
        self.assertEqual(response.getheader("Cache-Control"), "private, max-age=30")
