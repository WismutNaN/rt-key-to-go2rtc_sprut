...
3. В каталоге SprutHub выберите MQTT и импортируйте
   [`spruthub/rtkey_access_v2.json`](spruthub/rtkey_access_v2.json).
...
формат SprutHub с массивами `modelIds` и `link`. Если ранее использовался
`rtkey_access.json`, остановите MQTT-контроллер и удалите старый шаблон вместе
с найденными через него устройствами. Затем импортируйте файл с новым именем,
перезагрузите шаблоны, включите контроллер и выполните поиск/сопряжение устройств.

Каждый домофон или шлагбаум создаётся как управляемый переключатель `Open` —
дополнительный сценарий не нужен. Название Ростелекома передаётся в характеристику
`Name` и в гарантированно видимый параметр устройства `Provider name`; тип точки
показывается в `Access type`. Если версия SprutHub всё равно оставляет общее имя
`RT Key access point`, откройте параметры устройства, посмотрите `Provider name`
и один раз переименуйте его. То же соответствие печатают установщик и
`./manage.sh access`.
