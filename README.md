...
создаст каталог `generated/spruthub-access-<date>/` с одним JSON-файлом.
Импортируйте этот файл в каталог MQTT SprutHub, перезагрузите шаблоны, включите
контроллер и выполните поиск/сопряжение устройств.

Шаблон создаёт одно составное MQTT-устройство. Внутри него каждое место
Ростелекома становится отдельной кнопкой-переключателем со своим названием.
Название составного устройства содержит список мест. Дополнительный сценарий и
ручное переименование не нужны. Если названия совпадают, генератор добавляет к ним
разные короткие стабильные идентификаторы. Соответствия повторно показывает
`./manage.sh access`, а новый JSON создаёт `./manage.sh access-templates`.
...
устройствами. Затем импортируйте новый файл. Его имя зависит от содержимого,
...
./manage.sh access-templates # create one JSON with all access buttons
