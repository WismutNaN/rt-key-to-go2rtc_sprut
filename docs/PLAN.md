...
`./manage.sh down` останавливает Compose без удаления закрытого `.env` и volume.
...
Использовать предыдущий Git-тег. Не удалять volume и закрытый `.env` до подтверждения стабильной работы выбранной версии.
